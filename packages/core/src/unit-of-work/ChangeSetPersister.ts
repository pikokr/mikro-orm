...
export class ChangeSetPersister {
  ...
  async executeInserts<T>(changeSets: ChangeSet<T>[], options?: DriverMethodOptions, withSchema?: boolean): Promise<void> {
    if (!withSchema) {
  ...
  async executeUpdates<T>(changeSets: ChangeSet<T>[], batched: boolean, options?: DriverMethodOptions, withSchema?: boolean): Promise<void> {
    if (!withSchema) {
  ...
  async executeDeletes<T>(changeSets: ChangeSet<T>[], options?: DriverMethodOptions, withSchema?: boolean): Promise<void> {
    if (!withSchema) {
  ...
  private async runForEachSchema<T>(changeSets: ChangeSet<T>[], method: string, options?: DriverMethodOptions, ...args: unknown[]): Promise<void> {
  ...
  private processProperties<T>(changeSet: ChangeSet<T>): void {
  ...
  private async persistNewEntity<T>(meta: EntityMetadata<T>, changeSet: ChangeSet<T>, options?: DriverMethodOptions): Promise<void> {
  ...
  private async persistNewEntities<T>(meta: EntityMetadata<T>, changeSets: ChangeSet<T>[], options?: DriverMethodOptions): Promise<void> {
    const size = this.config.get('batchSize');
  ...
  private async persistNewEntitiesBatch<T>(meta: EntityMetadata<T>, changeSets: ChangeSet<T>[], options?: DriverMethodOptions): Promise<void> {
    options = this.propagateSchemaFromMetadata(meta, options, {
      convertCustomTypes: false,
  ...
  private async persistManagedEntity<T>(changeSet: ChangeSet<T>, options?: DriverMethodOptions): Promise<void> {
    const meta = this.metadata.find(changeSet.name)!;
  ...
  private async persistManagedEntities<T>(meta: EntityMetadata<T>, changeSets: ChangeSet<T>[], options?: DriverMethodOptions): Promise<void> {
    const size = this.config.get('batchSize');
  ...
  private checkConcurrencyKeys<T>(meta: EntityMetadata<T>, changeSet: ChangeSet<T>, cond: Dictionary): void {
    const tmp: string[] = [];
  ...
  private async persistManagedEntitiesBatch<T>(meta: EntityMetadata<T>, changeSets: ChangeSet<T>[], options?: DriverMethodOptions): Promise<void> {
   ...
   */
  private markAsPopulated<T>(changeSet: ChangeSet<T>, meta: EntityMetadata<T>) {
  ...
  private async updateEntity<T>(meta: EntityMetadata<T>, changeSet: ChangeSet<T>, options?: DriverMethodOptions): Promise<QueryResult<T>> {
    options = this.propagateSchemaFromMetadata(meta, options, {
      convertCustomTypes: false,
    });
  ...
  private async checkOptimisticLocks<T>(meta: EntityMetadata<T>, changeSets: ChangeSet<T>[], options?: DriverMethodOptions): Promise<void> {
  ...
  private checkOptimisticLock<T>(meta: EntityMetadata<T>, changeSet: ChangeSet<T>, res?: QueryResult) {
   ...
   */
  private async reloadVersionValues<T>(meta: EntityMetadata<T>, changeSets: ChangeSet<T>[], options?: DriverMethodOptions) {
  ...
  private processProperty<T>(changeSet: ChangeSet<T>, prop: EntityProperty<T>): void {
    const meta = this.metadata.find(changeSet.name)!;
   ...
   */
  private mapReturnedValues<T>(changeSet: ChangeSet<T>, res: QueryResult<T>, meta: EntityMetadata<T>): void {
